...
class TestTaggingSheetsIntegration:
    ...
    def test_tagging_saves_to_correct_sheet(self):
        ...
        assert response.status_code == 200, f"Failed to get records: {response.status_code}"
        ...
        print(f"   Original Tagger_1_Result: {untagged_record.get('Tagger_1_Result', 'None')}")
        ...
        assert tagged_record is not None, f"Could not find tagged record with link: {original_link}"
    ...
    def test_multiple_sheets_tagging(self):
        ...
        for sheet, sheet_records in records_by_sheet.items():
            untagged_count = sum(1 for r in sheet_records if not r.get('Tagger_1') or r.get('Tagger_1') == "")
        ...
        for sheet, sheet_records in records_by_sheet.items():
            ...
            for record in sheet_records:
                if (not record.get('Tagger_1') or record.get('Tagger_1') == "") and sheet not in used_sheets:
                    test_records.append(record)
                    used_sheets.add(sheet)
                    break
        
        if len(test_records) < 2:
            pytest.skip("Need at least 2 untagged records from different sheets to run this test")
    ...
    def test_get_sheets_statistics(self):
        ...
        assert len(sheets_in_data) > 1, f"Expected multiple sheets, but only found: {sheets_in_data}"
